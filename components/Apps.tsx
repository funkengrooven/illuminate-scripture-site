...
export default function Apps() {
  ...
  const apps = [
    ...
    {
      name: "Scriptura.AI",
      description: "An advanced AI-powered research assistant for scholars and serious students of scripture. Deep textual analysis meets intelligent study tools.",
      features: ["Contextual analysis", "Cross-references", "Historical insights", "Language tools", "Research automation"],
      status: "Coming Soon",
      comingSoon: true,
      ...
      <div className="container-max section-padding relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {apps.map((app, index) => (
            <div 
              key={index} 
              className={`group bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 p-8 card-hover shadow-lg transition-all duration-1000 ${app.delay} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'} relative`}
            >
              {/* Coming Soon Badge */}
              {app.comingSoon && (
                <div className="absolute -top-3 -right-3 bg-gradient-to-r from-accent-500 to-accent-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg animate-pulse">
                  Coming Soon
                </div>
              )}
        ...
        <div className={`text-center mt-20 transition-all duration-1000 delay-800 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="bg-gradient-to-r from-primary-50 to-accent-50 rounded-2xl p-8 border border-primary-100">
            ...
            <button className="bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-medium px-8 py-4 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 group">
